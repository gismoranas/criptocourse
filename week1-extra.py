...
from collection_utils import find_most_common_entries

#TODO move msg to other file and read it, so it can be accidentally modified
...
#  '''Returns a list with the entires with maximum cardinality.'''
#  counter = Counter(some_collection).most_common()
#  max_cardinality = counter[0][1]
#  return [ a[0] for a in counter if a[1] == max_cardinality ]
...
def easy_decrypt(chosen_index):
  ...
  for index, letters in index_to_letters.iteritems():    
    most_common = find_most_common_entries(letters)
    if most_common[1] > 2 and len(most_common[0]) == 1:
        plaintext[index] = most_common[0][0]

#    counter = Counter(letters).most_common()
